...
def firstTimeReadIn():
    ...
    newMovie = movieData.filter(lambda row: float(row.get("revenue"))>=1000 and row.get("status") == "Released" and creditIDs.has(row.get("id")));
    def addDate(movie, i, ary):
        toDate = movie.get("release_date");
        movie.set("year", int(str(toDate.slice(0,4))));
        movie.set("month", int(str(toDate.slice(5,7))));
    newMovie.forEach(lambda movie, i, ary: addDate(movie, i, ary));
...
crewFreqCount.createColumns(newMovie, lambda movie: movie.get("crew"));
...
#month
monthMap = JS.Map([[1,"Jan"],[2,"Feb"],[3,"Mar"],[4,"Apr"],[5,"May"],[6,"Jun"],[7,"Jul"],[8,"Aug"],[9,"Sep"],[10,"Oct"],[11,"Nov"],[12,"Dec"]]);
def createMonth(item, idx,ary):
    monthMap.forEach(lambda v,k,m: item.set(v, 0));
    item.set(monthMap.get(item.get("month")),1);
newMovie.forEach(createMonth);
...
header = headerPre.map(lambda k: k if JS.String(k).indexOf(",") == -1 else ("\"" + k + "\""));
progress = 5; totalL = newMovie.length; dumpable = JS.Array(["Map", "Set", "Array"]);
with open('tempData5.csv', 'w', newline='', encoding='utf-8-sig') as csvfile:
        toWriter = csv.writer(csvfile);
        toWriter.writerow(header);
        for i,row in enumerate(newMovie):
            toWriter.writerow([(repr(row.get(h)) if dumpable.includes(JS.classof(row.get(h))) else str(row.get(h))) for h in headerPre]);
